...
const Restaurants = () => {
  ...
  const ResCategoryMenu =
    resInfo?.data?.cards[4]?.groupedCard?.cardGroupMap?.REGULAR?.cards.filter((r) => r.card.card['@type'] === 'type.googleapis.com/swiggy.presentation.food.v2.ItemCategory');
    ...
    <div className="justify-center">
      <div className="flex justify-center">
        <div className="text-center mt-5  p-5">
          ...
          <h4 className="bg-yellow-400 rounded-full mt-5">{avgRating} Ratings</h4>
        </div>
      ...
      <div className="m-auto border-2 flex flex-col items-center text-center justify-center">
        {
          ResCategoryMenu.map((resCategory, index) => {
            return (
              <RestaurantCategory key= {index} data={resCategory?.card?.card} />
            )
          })
        }
      </div>
